...
public class Barrel_SVT {
	...
	public void fillBarrel(DataBank pbank, boolean isMC) {
		...
		for (int row=0;row<pbank.rows();row++){
			...
			if (ADC>0&&strip>0) Modules[layer-1][sector-1].addHit(strip, (end.x()+begin.x())/2., (end.y()+begin.y())/2., (end.z()+begin.z())/2., (phi_begin+phi_end)/2., Math.abs((phi_begin-phi_end)/2.), (end.z()-begin.z())/2., ADC, time);
		}
		MakeClusters();
		//PrintClusters();
	}
